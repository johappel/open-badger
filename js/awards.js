jQuery(function($) {
	...
	if (/MSIE (\d+\.\d+);/.test(navigator.userAgent)) {  //The Issuer API isn't supported on MSIE browsers
		$('#openbadger-award-actions').hide();
		$('#openbadger-award-browser-support').show();
	}
	...
	$('.acceptBadge').click(function(event) {
		...
		OpenBadges.issue(['' + assertionUrl + ''], function(errors, successes) {
			if (errors.length > 0) {
				var $errorsdiv = $('#openbadger-award-actions-errors');
				$.each(errors, function(idx, val) {
					console.log(val);
					if(val.reason == 'EXISTS'){
						$.post(
							WPBadger_Awards.ajaxurl,
							{
								action: 'openbadger_award_ajax',
								award_id: WPBadger_Awards.award_id,
								award_action: 'accept'
							},
							function(data, textStatus) {
								$('#openbadger-award-actions-wrap').html(data);
							}
						);
					}
