...
add_action( 'wp_ajax_openbadger_get_awards_stats', 'openbadger_badge_get_awards_stats' );
add_action( 'admin_menu', 'openbadger_badges_stats_admin_menu' );

function openbadger_badge_get_awards_stats()
{
    ...
    {
        $award_status = get_post_meta( $query->post->ID, 'openbadger-award-status', true );
...
class OpenBadger_Badges_Stats_List_Table extends WP_List_Table
{
    var $sort_key = 'title';
    var $sort_dir = 'ASC';
    ...
    function prepare_items()
    {
        ...
        while ($query->next_post())
        {
            $badge_id = (int)get_post_meta( $query->post->ID, 'openbadger-award-choose-badge', true );
            $award_status = get_post_meta( $query->post->ID, 'openbadger-award-status', true );
...
function openbadger_badges_stats_admin_menu()
{
    $badge_type = get_post_type_object( 'badge' );

    add_submenu_page(
        'edit.php?post_type=badge',
        'OpenBadger | Statistics',
        'Statistik',
        $badge_type->cap->edit_posts,
        'openbadger_badges_stats_page',
        'openbadger_badges_stats_page'
    );
}

function openbadger_badges_stats_page()
{
    $table = new OpenBadger_Badges_Stats_List_Table( $_REQUEST[ 'orderby' ], $_REQUEST[ 'order' ] );
    $table->prepare_items();
