...
# Award data
$email = get_post_meta( $post->ID, 'openbadger-award-email-address', true );
$salt = get_post_meta( $post->ID, 'openbadger-award-salt', true );
$issued_on = get_the_date('Y-m-d');
$evidence = get_permalink();

# Badge data
$badge_id = get_post_meta( $post->ID, 'openbadger-award-choose-badge', true );
$badge_title = get_the_title( $badge_id );
$badge_version = get_post_meta( $badge_id, 'openbadger-badge-version', true );
